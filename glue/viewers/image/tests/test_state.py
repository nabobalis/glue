...
def test_viewer_state_reference_data():

    # Regression test to ensure reference data is set correctly when
    # adding/removing 1D/2D layers

    viewer_state = ImageViewerState()

    data_1d = Data(x=np.arange(100))
    layer_state_1d = ImageLayerState(layer=data_1d, viewer_state=viewer_state)
    viewer_state.layers.append(layer_state_1d)

    # Verify that reference_data is None (1D data is filtered out)
    assert viewer_state.reference_data is None

    data_2d = Data(x=np.ones((5, 10)))
    layer_state_2d = ImageLayerState(layer=data_2d, viewer_state=viewer_state)
    viewer_state.layers.append(layer_state_2d)
    ...
    viewer_state.layers.append(another_layer_state_1d)

    # Verify that 2D data is still selected as reference
    assert viewer_state.reference_data is data_2d

    # Remove data layers from the viewer (start with 2D) and check that reference_data is None
    # Also confirms that nothing crashes or breaks along the way
    for layer_state in [layer_state_2d, another_layer_state_1d, layer_state_1d]:
        # Remove the layer
        viewer_state.layers.remove(layer_state)

        # Verify reference_data is None
        assert viewer_state.reference_data is None

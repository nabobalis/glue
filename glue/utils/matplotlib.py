...
def datetime64_to_mpl(d):
    """
    Convert `numpy.datetime64` values to days since `matplotlib.dates.get_epoch`
    (float64, so precise to about a microsecond for current dates).
    """
...
def mpl_to_datetime64(dt):
    """
    Convert days since `matplotlib.dates.get_epoch` back to `numpy.datetime64`.
    """
